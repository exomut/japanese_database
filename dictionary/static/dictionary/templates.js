...
const KanjiRowTemplate = ({kanji, id}) => {
	return `
    <div><span class="badge-pill badge-secondary mr-2">${id}</span>${kanji}</div>
    `;
};

const ReadingRowTemplate = ({reading, id}) => {
	return `
    <div><span class="badge-pill badge-secondary mr-2">${id}</span>${reading}</div>
    `;
};

const TranslationRowTemplate = ({trans, id}) => {
	return `
    <div><span class="badge-pill badge-primary mr-2">${id}</span> ${trans}</div>
    `;
};
